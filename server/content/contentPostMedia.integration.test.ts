...
import { contentPostMedia, contentPosts, productMedia, products, users } from "../../drizzle/schema";
...
describe("وسيط المنشور الاختياري للمنتج", () => {
  it("يبقى داخل المنشور افتراضيًا ولا يظهر في المنتج إلا بعد اختيار الإضافة الصريح", async () => {
    ...
    try {
      ...
      postId = await createContentPostDraft({ productId, caption: "مسودة اختبار", createdByUserId: owner.id });
      postMediaId = await saveContentPostMedia({ postId, storageKey: "content/posts/test/original.jpg", originalFileName: "external-high-quality.jpg", mimeType: "image/jpeg", byteSize: 128 });

      const beforeAttach = await getContentPostDraft(postId);
      ...
      const attached = await attachContentPostMediaToProduct({ postId, postMediaId, productId });
      linkedProductMediaId = attached.productMediaId;

      expect(attached).toMatchObject({ attached: true, postMediaRemainsIndependent: true });
      const afterAttach = await getContentPostDraft(postId);

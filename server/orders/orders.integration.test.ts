...
import { orderContactEvents, orderItems, orders, orderStatusEvents, productVariants, products, promotionCoupons, stores, users } from "../../drizzle/schema";
...
describe("دورة الطلب", () => {
  const cleanup: Array<{ orderId: number; productId: number; variantIds: number[]; couponId?: number; otherStoreId?: number }> = [];
  ...
  afterEach(async () => {
    ...
    for (const item of cleanup.splice(0)) {
      ...
      if (item.otherStoreId) await db.delete(stores).where(eq(stores.id, item.otherStoreId));
    }
  });
  ...
  it("ينشئ سلة بألوان متعددة بلا خصم، ويحفظ التسعير والتواصل ثم يخصم عند التأكيد ويعيد الكمية عند الإلغاء", async () => {
    ...
    expect((await getOperationalOrder(created.orderId, storeId))?.order).toMatchObject({ customerChannel: "instagram", total: "38000.00" });
    expect((await getOperationalOrder(created.orderId, storeId))?.contacts[0]).toMatchObject({ channel: "instagram", outcome: "attempted" });
    await transitionOrderStatus({ storeId, orderId: created.orderId, nextStatus: "confirmed", actorUserId: owner.id });
    ...
    const confirmed = await getOperationalOrder(created.orderId, storeId);
    expect(confirmed?.order.status).toBe("confirmed");
    expect(confirmed?.order.inventoryDeductedAt).not.toBeNull();
    await transitionOrderStatus({ storeId, orderId: created.orderId, nextStatus: "cancelled", actorUserId: owner.id });
    ...
    expect((await getOperationalOrder(created.orderId, storeId))?.events.map(event => event.toStatus)).toEqual(["cancelled", "confirmed", "new"]);
  });
  ...
  it("يطبق القسيمة على المنتجات فقط ويحفظ الخصم ويرفض تجاوز حد استخدامها", async () => {
    ...
    const couponResult = await db.insert(promotionCoupons).values({ storeId: store.id, code, discountType: "percent", discountValue: "25.00", minimumSubtotal: "5000.00", usageLimit: 1, enabled: true, createdByUserId: owner.id });
    ...
    expect((await getOperationalOrder(created.orderId, store.id))?.order).toMatchObject({ subtotal: "20000.00", deliveryFee: expectedDelivery.toFixed(2), manualDiscount: "5000.00", total: (15000 + expectedDelivery).toFixed(2), customerChannel: "storefront" });

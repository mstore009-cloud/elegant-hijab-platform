...
describe("Unified Meta Webhook Gateway", () => {
  it("يطبع Messenger ورسائل Instagram وحالات WhatsApp والتعليقات إلى أنواع أحداث موحدة", () => {
    const richMessenger = normalizeMetaEvents({ object: "page", entry: [{ id: "page-rich", messaging: [{ sender: { id: "customer-rich", name: "عميلة فيسبوك", username: "customer_fb" }, timestamp: 1760000000000, message: { mid: "mid-rich", text: "هل هذا رد على القصة؟", reply_to: { mid: "mid-root", text: "الصورة الأصلية" }, story_id: "story-1", mentions: [{ id: "staff-1", name: "موظفة" }] } }] }] });
    expect(richMessenger[0]).toMatchObject({ kind: "message", senderExternalId: "customer-rich", senderName: "عميلة فيسبوك", senderUsername: "customer_fb", externalThreadId: null, metadata: { messageType: "text", replyToExternalMessageId: "mid-root", replyToBodyPreview: "الصورة الأصلية", storyId: "story-1", mentions: [{ id: "staff-1", name: "موظفة" }] } });
    ...
    const messengerStandbyEcho = normalizeMetaEvents({ object: "page", entry: [{ id: "page-rich", standby: [{ sender: { id: "page-rich" }, recipient: { id: "customer-standby" }, timestamp: 1760000001500, message: { mid: "mid-echo-standby", text: "رد من صندوق الصفحة", is_echo: true } }] }] });
    expect(messengerStandbyEcho).toEqual([expect.objectContaining({ kind: "message", channel: "messenger", direction: "outbound", externalConversationId: "messenger:page-rich:customer-standby", externalMessageId: "mid-echo-standby", body: "رد من صندوق الصفحة" })]);

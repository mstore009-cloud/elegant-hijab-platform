...
export default function NativeInbox() {
  ...
  const enrichContact = trpc.inbox.enrichContact.useMutation({ onSuccess: () => void utils.inbox.detail.invalidate(), onError: () => undefined });
  ...
  useEffect(() => {
    ...
    const hasEnrichedIdentity = Boolean(current.customer?.profileImageUrl || current.customer?.socialUsername || current.conversation.contactAvatarUrl || current.conversation.contactUsername);
...
function ConversationRow({ item, active, onSelect }: { item: any; active: boolean; onSelect: () => void }) {
  ...
  const avatar = item.customer?.profileImageUrl || item.contactAvatarUrl;

...
import { CheckCheck, ChevronRight, ExternalLink, Filter, Inbox as InboxIcon, MapPin, MessageCircleMore, MoreHorizontal, Paperclip, Phone, Search, Send, Sparkles, UserRound, X } from "lucide-react";
...
function isGenericContactName(name?: string | null) { return !name || ["جهة اتصال", "عميل من المحادثات"].includes(name.trim()); }
function contactName(item: any) { const customerName = item?.customer?.displayName; return !isGenericContactName(customerName) ? customerName : item?.conversation?.contactNameSnapshot || item?.contactNameSnapshot || customerName || "جهة اتصال"; }
function contactAvatar(item: any) { return item?.customer?.profileImageUrl || item?.conversation?.contactAvatarUrl || item?.contactAvatarUrl || null; }

function safeNativeUrl(value?: string | null) {
  if (!value) return null;
  try {
    const url = new URL(value);
    return ["www.facebook.com", "facebook.com", "www.instagram.com", "instagram.com", "wa.me", "web.whatsapp.com"].includes(url.hostname) ? url.toString() : null;
  } catch {
    return null;
  }
}

function ContactAvatar({ name, src, size = "h-10 w-10" }: { name?: string | null; src?: string | null; size?: string }) {
  return src ? <img src={src} alt="" className={`${size} shrink-0 rounded-full object-cover ring-1 ring-black/5`} onError={event => { event.currentTarget.style.display = "none"; }} /> : <span className={`grid ${size} shrink-0 place-items-center rounded-full bg-[#e9eee9] text-sm font-bold text-[#4d6258]`}>{initial(name)}</span>;
}
...
export default function NativeInbox() {
  ...
  const contactEnrichmentRequested = useRef(new Set<number>());
  ...
  const enrichContact = trpc.inbox.enrichContact.useMutation({ onSuccess: result => { void utils.inbox.detail.invalidate(); if (!result.enriched && result.reason === "graph_profile_unavailable") toast.error("تعذر جلب اسم وصورة جهة الاتصال من Meta حالياً."); }, onError: error => toast.error(error.message) });
  ...
  useEffect(() => {
    if (!current?.conversation || current.conversation.channel === "whatsapp") return;
    const conversationId = current.conversation.id;
    const hasEnrichedIdentity = Boolean((current.customer?.displayName && !isGenericContactName(current.customer.displayName)) || current.customer?.profileImageUrl || current.customer?.socialUsername || current.conversation.contactNameSnapshot || current.conversation.contactAvatarUrl || current.conversation.contactUsername);
    if (hasEnrichedIdentity || contactEnrichmentRequested.current.has(conversationId)) return;
    contactEnrichmentRequested.current.add(conversationId);
    enrichContact.mutate({ conversationId });
  }, [current?.conversation?.id, current?.conversation?.channel, current?.conversation?.contactAvatarUrl, current?.conversation?.contactUsername, current?.customer?.profileImageUrl, current?.customer?.socialUsername]);
      ...
      <div className="flex min-h-[calc(100vh-10rem)] flex-col lg:grid lg:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <main className={`${showThreadMobile ? "flex" : "hidden lg:flex"} min-h-0 flex-col bg-[#fbfaf8]`}>
          {detail.isLoading ? <ThreadLoading /> : conversation ? <>
            <header className="flex items-center gap-3 border-b border-[#ebe5dc] bg-white px-4 py-3.5 sm:px-5"><Button variant="ghost" size="icon" onClick={() => setShowThreadMobile(false)} className="h-9 w-9 shrink-0 rounded-full lg:hidden" aria-label="العودة إلى المحادثات"><ChevronRight className="h-5 w-5" /></Button><ContactAvatar name={contactName({ customer: current?.customer, conversation })} src={contactAvatar({ customer: current?.customer, conversation })} /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="truncate text-sm font-bold text-[#35483f]">{contactName({ customer: current?.customer, conversation })}</h2><span className={`grid h-5 min-w-5 place-items-center rounded-full px-1 text-[10px] font-bold ${channelTone[conversation.channel as InboxChannel]}`} title={channelLabel[conversation.channel as InboxChannel]}>{channelIcon[conversation.channel as InboxChannel]}</span></div><p className="mt-0.5 truncate text-[11px] text-[#8a958e]">{current?.customer?.socialUsername || conversation.contactUsername || current?.customer?.phoneDisplay || (conversation.unreadCount > 0 ? `${conversation.unreadCount} رسائل غير مقروءة` : "")}</p></div><Button variant="outline" size="icon" onClick={() => setDetailsOpen(open => !open)} className={`h-9 w-9 rounded-full border-[#e8e1d8] ${detailsOpen ? "bg-[#f4e9eb] text-[#82495b]" : "bg-white text-[#63736a]"}`} aria-label="تفاصيل المحادثة"><MoreHorizontal className="h-5 w-5" /></Button></header>
...
function ConversationRow({ item, active, onSelect }: { item: any; active: boolean; onSelect: () => void }) {
  ...
  const avatar = contactAvatar(item);
  const last = item.latestMessage?.body || item.subject || "";
  const itemChannel = item.channel as InboxChannel;
  return <button type="button" onClick={onSelect} className={`w-full border-b border-[#f0ece7] px-4 py-3.5 text-right transition ${active ? "bg-[#f7eff1]" : "bg-white hover:bg-[#fcfaf8]"}`}><div className="flex items-start gap-2.5"><ContactAvatar name={name} src={avatar} /><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><b className={`truncate text-sm ${item.unreadCount > 0 ? "text-[#263b31]" : "text-[#4a5a52]"}`}>{name}</b><small className="shrink-0 text-[10px] text-[#949d96]">{formatTime(item.lastMessageAt || item.updatedAt)}</small></span><span className="mt-1 flex items-center gap-1.5"><span className="truncate text-xs text-[#77847c]">{last || " "}</span>{item.unreadCount > 0 && <i className="block h-2 w-2 shrink-0 rounded-full bg-[#c96d7c]" aria-label="غير مقروء" />}</span><span className="mt-1.5 flex items-center gap-1.5"><span className={`grid h-4 min-w-4 place-items-center rounded-full px-0.5 text-[8px] font-bold ${channelTone[itemChannel]}`}>{channelIcon[itemChannel]}</span>{item.customer?.socialUsername && <span className="truncate text-[10px] text-[#9aa49e]">@{item.customer.socialUsername}</span>}{item.hasAttachments && <Paperclip className="h-3 w-3 text-[#9b8472]" />}</span></span></div></button>;
}

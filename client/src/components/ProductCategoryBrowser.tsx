...
export function ProductCategoryBrowser({
  total, matchingTotal, primaryCategories, subcategories, selectedPrimaryCategory, selectedSubcategory,
  onSelectPrimary, onSelectSubcategory, search, onSearchChange, canCreate, canEdit, selectedProductId, currentCategoryId, onUpdated,
}: {
  total: number; matchingTotal: number; primaryCategories: CategoryCount[]; subcategories: CategoryCount[];
  selectedPrimaryCategory: string | null; selectedSubcategory: string | null;
  onSelectPrimary: (value: string | null) => void; onSelectSubcategory: (value: string | null) => void;
  search: string; onSearchChange: (value: string) => void; canCreate: boolean; canEdit: boolean;
  selectedProductId?: number | null; currentCategoryId?: number | null; onUpdated?: () => Promise<void> | void;
}) {
  ...
  return <div className="border-b border-[#eee9df] bg-[#fcfbf8] px-4 py-3 sm:px-5">
    <div className="flex flex-wrap items-center gap-2">
      <div className="relative w-full sm:w-52"><Search className="pointer-events-none absolute right-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#8a968f]" /><Input value={search} onChange={event => onSearchChange(event.target.value)} placeholder="ابحث بالاسم أو الكود" aria-label="البحث في المنتجات" className="h-8 rounded-lg border-[#ded8cd] pr-8 text-xs" /></div>
    ...
    {(selectedPrimaryCategory || selectedSubcategory || search.trim()) && <p className="mt-2 text-[11px] text-[#728078]">{matchingTotal} نتيجة مطابقة</p>}
  </div>;
}

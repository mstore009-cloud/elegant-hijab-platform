...
export function MetaCatalogWorkspace({ canEdit, onOpenProduct, onOpenSettings }: Props) {
  ...
  const [prepareResult, setPrepareResult] = useState<{
    prepared: number;
    preparedItems: Array<{ productId: number; mediaId: number; mediaType: "image" | "video"; source: string }>;
    skipped: Array<{ productId: number; mediaId: number; reason: string }>;
  } | null>(null);

  const selectedAsset = readiness.data?.assets.find(asset => asset.id === catalogAssetId)
    ?? readiness.data?.assets.find(asset => asset.isSelected)
  ...
  const prepare = trpc.metaCatalog.prepareProductsMedia.useMutation({
    onSuccess: async result => {
      ...
      await Promise.all([
        utils.metaCatalog.workspaceProducts.invalidate(),
        utils.metaCatalog.preview.invalidate(),
        utils.metaCatalog.sourceUpdates.invalidate(),
      ]);
      setStep("review");
    },
  });
  ...
  const sourceUpdateByProduct = useMemo(
    () => new Map((sourceUpdates.data ?? []).filter(update => update.status === "pending_review" || update.status === "media_prepared").map(update => [update.productId, update])),
    [sourceUpdates.data],
  );
  ...
  return <div className="space-y-4">
    <section className="rounded-2xl border border-[#d8e5de] bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="flex items-center gap-2 font-bold text-[#183d35]"><Layers3 className="h-4 w-4 text-[#a47d40]" />تصدير Meta Catalog</p>
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-[#f2f8f4] px-3 py-1.5 text-xs font-bold text-[#245b4d]">{selectedIds.length} منتج محدد</span>
          <span className={`rounded-full px-3 py-1.5 text-xs font-bold ${readyCount ? "bg-[#e4f3ea] text-[#17633b]" : "bg-[#f5f2eb] text-[#69736d]"}`}>{readyCount} جاهز</span>
          {needsReviewCount ? <span className="rounded-full bg-[#fff1de] px-3 py-1.5 text-xs font-bold text-[#a35d1c]">{needsReviewCount} يحتاج إصلاحًا</span> : null}
          <Button size="sm" variant="outline" onClick={onOpenSettings} className="border-[#d7e2dc] text-[#245b4d]"><Settings2 className="ml-1.5 h-3.5 w-3.5" />الإعدادات</Button>
        </div>
      </div>
      <div className="mt-3 flex gap-2">{stepLabels.map((item, index) => <span key={item.id} className={`rounded-full px-3 py-1.5 text-xs ${step === item.id ? "bg-[#183d35] font-bold text-white" : index < stepLabels.findIndex(stepItem => stepItem.id === step) ? "bg-[#e9f4ef] text-[#21624d]" : "bg-[#f6f5f1] text-[#74817a]"}`}>{index + 1}. {item.label}</span>)}</div>
    </section>

    {step === "select" ? <section className="rounded-2xl border border-[#d8e5de] bg-white p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h3 className="font-bold text-[#183d35]">المنتجات النشطة</h3><p className="mt-1 text-xs leading-5 text-[#68756e]">حدد منتجًا واحدًا أو أكثر. المسودات لا تظهر هنا ولا يمكن إرسالها إلى Meta.</p></div>
        <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={selectVisibleProducts} disabled={!visibleProducts.length} className="border-[#b9d3c6] text-[#245b4d]">تحديد الكل ({visibleProducts.length})</Button><Button size="sm" variant="ghost" onClick={() => setSelectedIds([])} disabled={!selectedIds.length} className="text-[#7a5a25]">إلغاء التحديد</Button></div>
      </div>
      <Input value={query} onChange={event => setQuery(event.target.value)} placeholder="ابحث بالاسم أو الكود" className="mt-4 h-10 max-w-md rounded-xl border-[#d8e5de]" />
      <div className="mt-3 overflow-hidden rounded-2xl border border-[#e6ece8]">
        <div className="grid grid-cols-[34px_minmax(150px,1.2fr)_minmax(100px,.8fr)_78px_104px] gap-2 bg-[#f7faf8] px-3 py-2 text-xs font-bold text-[#587066]"><span></span><span>المنتج</span><span>القسم</span><span>الوسائط</span><span>الحالة</span></div>
        {products.isLoading ? <p className="p-4 text-xs text-[#718178]">جارٍ تحميل المنتجات النشطة…</p> : visibleProducts.length ? visibleProducts.map(product => {
          const selected = selectedSet.has(product.id);
          const hasSourceUpdate = Boolean(sourceUpdateByProduct.get(product.id));
        ...
        }) : <p className="p-4 text-xs text-[#718178]">لا توجد منتجات نشطة مطابقة للبحث.</p>}
      </div>
      <div className="mt-4 flex justify-end"><Button onClick={() => setStep("complete")} disabled={!selectedIds.length} className="bg-[#183d35] text-white hover:bg-[#245b4d]">فحص المنتجات المحددة ({selectedIds.length})<ChevronLeft className="mr-1.5 h-4 w-4" /></Button></div>
    </section> : null}

    {step === "complete" || step === "review" ? <section className="rounded-2xl border border-[#d8e5de] bg-white p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><h3 className="font-bold text-[#183d35]">{step === "complete" ? "فحص المنتج وتجهيز الوسائط" : "معاينة ما سيُرسل"}</h3><p className="mt-1 text-xs leading-5 text-[#68756e]">الاسم والوصف والفئة والصور والفيديو المعروضة هنا هي القيم النهائية التي ستُرسل إلى Meta.</p></div>
        {selectedAssetId ? <label className="text-xs font-bold text-[#4d6158]">الكتالوج المستهدف<select value={selectedAssetId} onChange={event => setCatalogAssetId(Number(event.target.value))} className="mt-1 block h-9 rounded-lg border border-[#d8e5de] bg-white px-2 text-xs text-[#30483e]">{readiness.data?.assets.map(asset => <option key={asset.id} value={asset.id}>{asset.displayName ?? asset.externalId}</option>)}</select></label> : null}
      </div>
      {preview.isLoading || preview.isFetching ? <p className="mt-4 rounded-xl bg-[#f7faf8] p-3 text-xs text-[#718178]">جارٍ تحديث معاينة الحقول والوسائط قبل الإرسال…</p> : preview.error ? <p className="mt-4 rounded-xl border border-[#f0d6bc] bg-[#fff7ef] p-3 text-xs text-[#9c4b25]">تعذر فحص الدفعة: {preview.error.message}</p> : <div className="mt-4 space-y-2">{selectedProducts.map(product => {
        const report = reportByProduct.get(product.id);
        const isReady = report?.status === "ready";
        return <article key={product.id} className={`rounded-xl border p-3 ${isReady ? "border-[#cce0d7] bg-[#f6fbf8]" : "border-[#efd9ba] bg-[#fffaf1]"}`}>
          <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3">
      ...
      {exportNow.data ? <p className="mt-3 rounded-xl bg-[#eef7f2] p-3 text-xs font-bold text-[#21624d]"><Sparkles className="ml-1 inline h-4 w-4" />تم إرسال {exportNow.data.snapshot.itemCount} عنصرًا من المنتجات النشطة المحددة إلى Meta.</p> : null}

...
const channelLabels: Record<string, string> = { whatsapp: "واتساب", instagram: "إنستغرام", messenger: "Messenger", manual: "يدوية" };
const inboxStatusLabels: Record<string, string> = { open: "مفتوحة", waiting_customer: "بانتظار العميل", snoozed: "مؤجلة", closed: "مغلقة" };
          ...
          {detail.isLoading || (selectedCustomerId !== null && !current) ? <CustomerDetailSkeleton /> : detail.error ? <div className="p-6"><InlineError message="تعذر فتح ملف العميل. يمكنك العودة إلى القائمة واختيار ملف آخر." onRetry={() => detail.refetch()} /></div> : current ? <>
            ...
            <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[minmax(0,1.18fr)_minmax(260px,0.82fr)]">
              ...
              <aside className="space-y-5"><section className="rounded-2xl bg-[#153d35] p-4 text-white"><p className="text-xs text-[#b9d8cd]">ملخص العميل</p><div className="mt-4 grid grid-cols-2 gap-3"><SummaryItem label="عدد الطلبات" value={String(detail.data?.orders.length ?? 0)} /><SummaryItem label="إجمالي الطلبات" value={formatMoney(detail.data?.orders.reduce((sum, order) => sum + Number(order.total), 0).toFixed(2) ?? "0.00")} /></div><div className="mt-4 space-y-2 border-t border-white/10 pt-4 text-xs text-[#d7e7e0]"><p className="flex items-center gap-2"><Phone className="h-3.5 w-3.5 text-[#f0d492]" />{current.phoneDisplay}</p>{current.governorate && <p className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-[#f0d492]" />{current.governorate}</p>}{current.lastAddress && <p className="leading-5">{current.lastAddress}</p>}</div></section>
                <section className="rounded-2xl border border-[#eee8da] p-4"><div className="flex items-start justify-between gap-3"><SectionTitle icon={<MessageSquareText />} title="محادثات العميل" subtitle="المحادثات المرتبطة بهذا الملف داخل المتجر الحالي." /><a href="/inbox" className="shrink-0 rounded-lg border border-[#b9d8cd] px-2.5 py-1.5 text-xs font-semibold text-[#1f5b4f] hover:bg-[#edf5f1]">فتح Inbox</a></div><div className="mt-3 space-y-2">{detail.data?.conversations.length ? detail.data.conversations.map(conversation => <a key={conversation.id} href={`/inbox?conversation=${conversation.id}`} className="block rounded-xl bg-[#fbfaf6] p-3 transition-colors hover:bg-[#f3f7f4]"><div className="flex items-start justify-between gap-2"><p className="text-sm font-semibold text-[#425148]">{conversation.subject || `محادثة ${channelLabels[conversation.channel] ?? conversation.channel}`}</p>{conversation.priority && <Badge className="rounded-full bg-amber-50 text-[11px] text-amber-800">أولوية</Badge>}</div><div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs text-[#78867d]"><span>{channelLabels[conversation.channel] ?? conversation.channel}</span><span>{inboxStatusLabels[conversation.status] ?? conversation.status}</span><span>{formatDate(conversation.lastMessageAt ?? conversation.createdAt)}</span></div></a>) : <SmallEmpty text="لا توجد محادثات مرتبطة بهذا العميل حتى الآن." />}</div></section>

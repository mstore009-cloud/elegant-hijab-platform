...
export default function CustomerBotCommandAssistant({ embedded = false, canManageOverride }: { embedded?: boolean; canManageOverride?: boolean }) {
  ...
  const improveProposal = trpc.customerBot.improveCommand.useMutation({ onSuccess: async result => { await utils.customerBot.commandRequests.invalidate(); setSelected(result.command.id); setImprovementNote(""); toast.success("تم تحسين الاقتراح وعرض البدائل للمراجعة."); }, onError: error => toast.error(error.message) });
  ...
  }, [selectedCommand?.id, selectedCommand?.proposedChangeJson, selectedCommand?.classificationJson]);
  ...
  function applyAlternative(alternative: { destination: string; title: string; body: string }) {
    setDraftCategory(alternative.destination);
    setDraftTitle(alternative.title);
    setDraftBody(alternative.body);
    toast.success("تم اختيار البديل ويمكنك تعديله قبل حفظه.");
  }

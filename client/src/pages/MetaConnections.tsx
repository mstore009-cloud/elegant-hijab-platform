...
export default function MetaConnections() {
  ...
  return <div dir="rtl" className="mx-auto max-w-6xl space-y-5 pb-10">
    <header className="overflow-hidden rounded-[1.8rem] border border-[#eadfe2] bg-[radial-gradient(circle_at_85%_10%,#f8e8eb,transparent_38%),linear-gradient(135deg,#fffdfc,#f7f1ed)] px-5 py-6 shadow-[0_14px_35px_rgba(82,48,60,0.07)] sm:px-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[11px] font-bold tracking-[0.14em] text-primary">META INTEGRATION HUB</p><h1 className="mt-1.5 flex items-center gap-2 text-2xl font-black text-accent"><Cable className="h-6 w-6 text-primary" />مركز اتصال Meta</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">يربط المتجر عبر تسجيل دخول Meta الرسمي ومنح الصلاحيات واختيار الحسابات. لا يطلب من المستخدم Access Token أو App Secret.</p></div><Badge className={data?.configured ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}>{data?.configured ? "إعداد المنصة جاهز" : "ينتظر إعداد مسؤول المنصة"}</Badge></div>
    </header>

    {!data?.configured && <Alert className="border-amber-200 bg-amber-50/80 text-amber-950"><AlertTriangle className="h-4 w-4" /><AlertTitle>إعداد تطبيق المنصة ينتظر مسؤول النظام</AlertTitle><AlertDescription>هذا إعداد مركزي يُنجز مرة واحدة خارج حسابات المتاجر. بعد جاهزيته سيضغط صاحب المتجر «تسجيل الدخول وربط»، ثم يوافق في Meta ويختار صفحته وحساباته؛ لن يدخل أي سر أو رمز وصول.</AlertDescription></Alert>}

    <Card className="border-border/80 bg-card/95"><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" />طبقتان منفصلتان للربط</CardTitle><CardDescription>إعداد التطبيق مسؤولية مشغل المنصة مرة واحدة؛ ربط المتجر يتم عبر OAuth ولا يكشف الرمز للواجهة.</CardDescription></CardHeader><CardContent className="grid gap-3 text-sm sm:grid-cols-3"><StatusItem label="إعداد مشغل المنصة" value={data?.configured ? "جاهز لتسجيل الدخول" : "غير مكتمل بعد"} /><StatusItem label="تجربة صاحب المتجر" value="تسجيل الدخول · الموافقة · اختيار الأصول" /><StatusItem label="الإرسال والنشر" value="لا يتفعّل تلقائياً بعد الربط" /></CardContent></Card>
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {(Object.keys(purposeMeta) as Purpose[]).map(purpose => {
        ...
        return <Card key={purpose} className="overflow-hidden border-border/80 bg-card/95 shadow-[0_12px_28px_rgba(82,48,60,0.05)]"><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-secondary text-primary"><Icon className="h-5 w-5" /></span><ConnectionBadge status={connection?.status} /></div><CardTitle className="pt-2 text-base">{meta.title}</CardTitle><CardDescription className="leading-5">{meta.description}</CardDescription></CardHeader><CardContent className="space-y-3">
          <div className="rounded-xl bg-secondary/70 p-3 text-xs leading-5 text-muted-foreground"><p>طريقة الربط: تسجيل الدخول الرسمي ومنح الصلاحيات</p><p>آخر تحقق: {connection?.lastVerifiedAt ? new Date(connection.lastVerifiedAt).toLocaleString("ar-IQ") : "لا يوجد"}</p>{!purposeConfig?.configurationIdConfigured && data?.configured && <p className="mt-1 text-amber-700">سيستخدم هذا الغرض الصلاحيات الافتراضية المعتمدة.</p>}{connection?.lastError && <p className="mt-1 text-destructive">{connection.lastError}</p>}</div>
          ...
          <div className="flex flex-wrap gap-2"><Button size="sm" disabled={!data?.configured || begin.isPending} onClick={() => begin.mutate({ purpose })} className="rounded-lg"><ExternalLink className="ml-1.5 h-3.5 w-3.5" />{connection ? "تسجيل الدخول وإعادة الربط" : "تسجيل الدخول والربط"}</Button>{connection && <Button size="sm" variant="outline" disabled={refresh.isPending} onClick={() => refresh.mutate({ purpose })} className="rounded-lg"><RefreshCw className="ml-1.5 h-3.5 w-3.5" />تحديث الحسابات</Button>}{connection && <Button size="sm" variant="outline" disabled={disconnect.isPending} onClick={() => window.confirm("هل أنت متأكد من إبطال هذا الاتصال؟") && disconnect.mutate({ purpose, confirm: true })} className="rounded-lg border-destructive/30 text-destructive"><Unplug className="ml-1.5 h-3.5 w-3.5" />إبطال</Button>}</div>
        </CardContent></Card>;
      })}
    </section>
  </div>;

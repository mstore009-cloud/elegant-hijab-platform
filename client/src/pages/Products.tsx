...
export default function Products() {
  ...
  const activateCatalogScan = trpc.catalogSync.activate.useMutation({ onSuccess: () => syncStatus.refetch() });
  ...
  return (
    <div dir="rtl" className="mx-auto w-full max-w-7xl space-y-6 pb-10">
      ...
      {catalogStatus.data?.status === "catalog_selected" && canCreate && <section className="flex flex-col gap-3 rounded-2xl border border-[#cfe1d7] bg-[#f3faf6] p-5 text-[#245946] sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">الفحص التلقائي في الخلفية</p><p className="mt-1 text-sm">{syncStatus.data?.scheduleCronTaskUid ? "الفحص الدوري مفعّل؛ ستظهر المسودات الجديدة تلقائيًا." : "بعد نشر الموقع، فعّل الفحص كل عشر دقائق مرة واحدة."}</p></div><Button onClick={() => activateCatalogScan.mutate({ cronExpression: "0 */10 * * * *" })} disabled={!import.meta.env.PROD || Boolean(syncStatus.data?.scheduleCronTaskUid) || activateCatalogScan.isPending} className="bg-[#1f5b4f] hover:bg-[#153d35]">{activateCatalogScan.isPending ? "جارٍ التفعيل..." : syncStatus.data?.scheduleCronTaskUid ? "الفحص الدوري مفعّل" : import.meta.env.PROD ? "تفعيل الفحص كل 10 دقائق" : "يتاح بعد النشر"}</Button>{activateCatalogScan.error && <p className="text-sm text-[#9c4b25]">{activateCatalogScan.error.message}</p>}</section>}
